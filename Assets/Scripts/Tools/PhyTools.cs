...
namespace Tools
{
    public static class PhyTools
    {
        ...
        // * Every method here will draw the cast if Debug is On in the Game Settings (Assets -> Resources -> GameSettingsObject -> Debug)
        ...
        public static bool RaycastForTransform(Vector3 position, Vector3 direction, float distance, Transform entity, Color debugColor = default) {
            var hit = Raycast(position, direction, distance, debugColor);
            return hit && hit.transform && hit.transform == entity;
        }

        /// Returns a list of Transforms that are caught in the given Boxcast
        ...
        /// Returns a list of colliders that are caught in the Overlap Box
        public static Collider[] OverlapBox(Vector3 center, Vector3 halfExtents, Color debugColor = default)
        {
            var hits = new Collider[3];
            _ = GameSettings.Debug
                ? DrawPhysics.OverlapBoxNonAlloc(center, halfExtents, hits) 
